...
# 알림 보낼 채널 ID 저장 변수 (기본 환경변수 또는 명령어로 동적 설정)
...
class ProfileModal(discord.ui.Modal, title='자기소개 입력'):
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    try:
      ...
      nick_msg = (
          '⚠️ (봇 권한 부족 또는 최고권한자 계정이라 닉네임 수정은 건너뛰었습니다.)'
      )
...
@bot.tree.command(
    name='사다리',
    description=(
        '현재 통화방 인원으로 당첨 인원수를 지정하여 사다리 타기를 진행합니다.'
    ),
)
...
async def ladder_game(
  ...
  if total_count < 2:
    await interaction.response.send_message(
        f'❌ **[{voice_channel.name}]** 통화방에 최소 2명 이상의 인원이 있어야 사다리를 탈 수'
        ' 있습니다!',
        ephemeral=True,
    )
    return
  ...
  if 당첨인원 <= 0:
    await interaction.response.send_message(
        '❌ 당첨 인원은 최소 1명 이상이어야 합니다!', ephemeral=True
    )
    return
  if 당첨인원 >= total_count:
    await interaction.response.send_message(
        f'❌ 당첨 인원({당첨인원}명)이 통화방 전체 인원({total_count}명)보다 많거나 같을 수'
        ' 없습니다.',
        ephemeral=True,
    )
    return
  ...
  embed.set_footer(
      text=(
          f'주최: {interaction.user.display_name} • 총 {total_count}명 중'
          f' {당첨인원}명 당첨'
      )
  )
...
async def daily_gold_reminder():
  ...
  # 설정된 채널이 없거나 삭제되었으면 시스템/기본 텍스트 채널로 대체
  if not channel:
