...
async def process_job_selection(
    interaction: discord.Interaction, job_name: str
):
...
class NoticeModal(discord.ui.Modal, title='📢 공지사항 작성'):
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    if self.notice_footer.value:
      embed.add_field(
          name='📌 전달사항', value=self.notice_footer.value, inline=False
      )
    embed.set_footer(
        text=(
            f'작성자: {interaction.user.display_name} •'
...
class PollButton(discord.ui.Button):
  ...
  async def callback(self, interaction: discord.Interaction):
    ...
    await interaction.response.edit_message(
        embed=view.make_embed(), view=view
    )
...
class PollView(discord.ui.View):
  ...
  def __init__(
      self, question: str, options: List[str], author: discord.Member
  ):
    super().__init__(timeout=None)
  ...
  def make_embed(self) -> discord.Embed:
    ...
    embed.set_footer(
        text=(
            f'주최: {self.author.display_name} • 총 {total_votes}명 참여 (버튼으로'
            ' 투표/변경)'
        )
    )
    return embed
...
class LoanView(discord.ui.View):
  ...
  @discord.ui.button(
      ...
      emoji='✅',
      custom_id='loan_return_btn',
  )
  async def return_button(
      self, interaction: discord.Interaction, button: discord.ui.Button
  ):
    if (
        interaction.user.id != self.lender_id
        and interaction.user.id != self.borrower_id
        and not is_admin_or_mod(interaction)
    ):
      await interaction.response.send_message(
          '❌ 빌려준 사람, 빌린 사람 또는 관리자만 반납 처리를 할 수 있습니다.',
          ephemeral=True,
      )
      return
    ...
    for i, field in enumerate(new_embed.fields):
      if field.name == '📌 상태':
        new_embed.set_field_at(
            i, name='📌 상태', value='✅ **반납 완료**', inline=True
        )
        break

    new_embed.add_field(
        name='🎉 반납 일시',
        value=f'{now_str} (확인: {interaction.user.display_name})',
        inline=False,
    )
    ...
    await interaction.followup.send(
        f'✅ **{interaction.user.display_name}** 님이 상환 완료 처리했습니다!',
        ephemeral=False,
    )
...
@bot.tree.command(
    name='자기소개',
    description='이름, 마크 닉네임, 출생 연도를 입력하여 닉네임을 설정합니다.',
)
...
@bot.tree.command(
    name='직업선택', description='버튼 형태의 직업 선택 메뉴를 출력합니다.'
)
async def job_select_menu(interaction: discord.Interaction):
  embed = discord.Embed(
      title='⚔️ 직업 선택',
      description=(
          '아래 버튼을 누르면 닉네임 앞에 `[직업]` 태그가 붙습니다!'
      ),
...
@bot.tree.command(
    name='직업', description='직업 이름을 직접 입력하여 변경합니다.'
)
...
@bot.tree.command(
    name='직업추가', description='선택 목록에 새로운 직업을 추가합니다.'
)
...
async def add_job(interaction: discord.Interaction, 직업명: str):
  ...
  if 직업명 in JOBS:
    await interaction.followup.send(
        f'❌ **{직업명}**은(는) 이미 존재합니다.', ephemeral=True
    )
    return
  JOBS.append(직업명)
  await interaction.followup.send(
      f'✅ 직업 **[{직업명}]**이(가) 추가되었습니다!', ephemeral=True
  )


@bot.tree.command(
    name='직업삭제', description='선택 목록에서 직업을 삭제합니다.'
)
...
async def remove_job(interaction: discord.Interaction, 직업명: str):
  ...
  JOBS.remove(직업명)
  await interaction.followup.send(
      f'🗑️ 직업 **[{직업명}]**이(가) 삭제되었습니다!', ephemeral=True
  )


@bot.tree.command(
    name='사다리', description='통화방 인원으로 사다리 타기를 진행합니다.'
)
...
@bot.tree.command(
    name='공지', description='팝업창을 열어 공지사항을 작성합니다.'
)
...
async def clear_messages(interaction: discord.Interaction, 개수: int):
  ...
  try:
    ...
    await interaction.followup.send(
        f'🧹 메시지 **{len(deleted)}개**를 삭제했습니다.', ephemeral=True
    )
  except discord.Forbidden:
    await interaction.followup.send(
        '⚠️ 메시지 관리 권한이 없습니다.', ephemeral=True
    )


@bot.tree.command(
    name='투표', description='버튼식 실시간 투표를 생성합니다. (최대 5개)'
)
...
@bot.tree.command(
    name='대여', description='[은행] 돈이나 물건을 빌려준 내역 박스를 생성합니다.'
)
@app_commands.describe(
    빌린사람='돈이나 물건을 빌려간 멤버',
...
async def loan_card(
  ...
  if 빌린사람.id == interaction.user.id:
    await interaction.response.send_message(
        '❌ 자기 자신에게 빌려줄 수는 없습니다!', ephemeral=True
    )
    return
  ...
  embed.add_field(
      name='👤 빌려준 사람', value=interaction.user.display_name, inline=True
  )
  embed.add_field(
      name='🙋 빌린 사람', value=빌린사람.display_name, inline=True
  )
  embed.add_field(name='📌 상태', value='⏳ **미반납**', inline=True)
  if 비고:
    embed.add_field(name='📝 메모', value=비고, inline=False)
  embed.set_footer(
      text=f'등록 일시: {now_str} • 상환 완료 후 아래 버튼을 클릭하세요.'
  )

  view = LoanView(lender_id=interaction.user.id, borrower_id=빌린사람.id)
  await interaction.response.send_message(
      content=f'{빌린사람.mention}', embed=embed, view=view
  )
...
async def check_warnings(
  ...
  lines = [
      f"**{i}.** {r['reason']} *({r['moderator']} • {r['date']})*"
...
@bot.tree.command(
    name='경고차감', description='[관리자 전용] 경고를 차감합니다.'
)
...
@bot.tree.command(
    name='경고초기화', description='[관리자 전용] 경고를 초기화합니다.'
)
...
async def manual_sync(ctx):
  try:
    ...
    await ctx.send(
        f'✅ 이 서버에 **{len(synced)}개**의 슬래시 명령어를 즉시'
        ' 동기화했습니다! 잠시 후 `/`를 쳐보세요.'
    )
